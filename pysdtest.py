...
@author: Kyungho Lee (SNU), Siwon Ryu (Rice)
...
class test_sd :
    ...
    contact : boolean
                whether to use 'Contact-set' estimation
    ...
    def bootstrap(self, sample, btspsize, nbtsp):
        ...
        btspindex = np.array([random.sample(list(np.arange(0, n, 1)), btspsize) for _ in np.arange(nbtsp)]).T[:,None,:]
...
class test_sd_SR :
    ...
    Stochastic Dominance Testing in Donald and Hsu (Econemtrics Review)
...
class test_sd_NDM :
    ...
    def testing(self) :
        ...
        print('\n#-------------------------------------------#')    
        print('Testing Stochastic Dominance by Numerical Delta Method',
          '\n\n* H0 : sample1 ', Torder, 'sample2')
