program pysdtest, rclass
    ...
    if "`byvar'" == "" {
		...
		python: run_test_sd("`var1'", "`var2'", "`touse'", "`approach'", `ngrid', `s', "`resampling'", `b1', `b2', `nboot', 								`alpha', `c', `a', `eta', `epsilon', "`form'")
		}
		else {
	...
	* if by( ) is specified
    ...
    local nlevels : word count `levels'
		...
		// Call Python function
		if ("`approach'" == "") | ("`approach'" == "contact") | ("`approach'" == "SR") | ("`approach'" == "NDM")  {
		python: run_test_sd_by("`var'", "`byvar'", "`level1'", "`level2'", "`touse'", "`approach'", `ngrid', `s', 								"`resampling'", `b1', `b2', `nboot', `alpha', `c', `a', `eta', `epsilon', "`form'")
		}
		else {
			display as error "Specify a correct approach. It needs to be one of empty string '', 'contact', 'SR', or 'NDM'"
		}	
...
def run_test_sd(var1, var2, touse, approach, ngrid, s, resampling, b1, b2, nboot, alpha, c, a, eta, epsilon, form):
	...
	if approach == "":
		...
		run_sd = pysdtest.test_sd_NDM(sample1, sample2, ngrid, s, resampling, b1, b2, nboot, epsilon, form, alpha)
...
def run_test_sd_by(var, byvar, level1, level2, touse, approach, ngrid, s, resampling, b1, b2, nboot, alpha, c, a, eta, epsilon, form):
